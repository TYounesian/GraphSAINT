...
class my_HighOrderAggregator(nn.Module):
    def __init__(self, dim_in, dim_out, dropout=0., act='relu', \
        ...
        Outputs:
            None
        """
        super(my_HighOrderAggregator,self).__init__()
    ...
    def _f_feat_trans(self, _feat, _id):
        #pdb.set_trace()
        feat = self.f_lin[_id](_feat) + self.f_bias[_id]
        if self.bias == 'norm':
    ...
    def forward(self, inputs):
        ...
        feat_partial = [self._f_feat_trans(ft, 0) for idf, ft in enumerate(feat_hop)]
        if self.aggr == 'mean':
        ...
        return adj_norm, self.act(feat_out)       # return adj_norm to support Sequential
